...
offer_auction = on_command("内卷", priority=5, permission= GROUP)
...
2、使用+物品名字：使用物品
3、内卷+金额：对本次内卷会的物品进行内卷

非指令：
1、定时生成内卷会，每天{auction_time_config['hours']}点每整点生成一场内卷会
""".strip()
...
async def _():
    ...
    if groups != []:
        ...
        if auction != {}:#存在内卷会
            ...
            msg += f"\n底价为{start_price}灵石"
            msg += "\n请诸位道友发送 内卷+金额 来进行内卷吧！"
            msg += f"\n本次内卷时间为：{AUCTIONSLEEPTIME}秒！"
            ...
            while auction_offer_flag:#有人交友
                if auction_offer_all_count == 0:
                    ...
                    break
                logger.info(f"有人内卷，本次等待时间：{auction_offer_all_count * AUCTIONOFFERSLEEPTIME}秒")
                first_time = auction_offer_all_count * AUCTIONOFFERSLEEPTIME
                auction_offer_all_count = 0
                auction_offer_flag = False
                await asyncio.sleep(first_time)
                logger.info(f"总计等待时间{auction_offer_time_count * AUCTIONOFFERSLEEPTIME}秒，当前内卷标志：{auction_offer_flag}，本轮等待时间：{first_time}")
            ...
            if user_stone < now_price:
                msg = f"内卷会结算！竞拍者灵石小于内卷，判定为捣乱，捣乱次数+1！"
                for group_id in groups:
                    try:
                        if XiuConfig().img:
                            pic = await get_msg_pic(msg)
                ...
                return
            msg = "本次内卷会结束！"
            msg += f"恭喜来自群{auction['group_id']}的{user_info.user_name}道友成功内卷获得：{auction['type']}-{auction['name']}！"
            for group_id in groups:
                try:
                    if XiuConfig().img:
                        pic = await get_msg_pic(msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if price - now_price < min_price:
        msg = f"内卷不得少于当前竞拍价的5%，目前最少加价为：{min_price}灵石，目前竞拍价为：{now_price}！"
        if XiuConfig().img:
            pic = await get_msg_pic(msg)
            await offer_auction.finish(MessageSegment.image(pic), at_sender=True)
        else:
    ...
    msg = f"来自群{group_id}的{user_info.user_name}道友内卷：{price}枚灵石！" \
        f"内卷时间增加：{AUCTIONOFFERSLEEPTIME}秒，内卷剩余时间：{int(AUCTIONSLEEPTIME - dif_time + AUCTIONOFFERSLEEPTIME * auction_offer_time_count)}秒"
    error_msg = ''
    for group_id in groups:
        try:
            if XiuConfig().img:
            ...
            error_msg = f"消息发送失败，可能被风控，当前内卷物品金额为：{auction['now_price']}！"
            continue
    logger.info(f"有人内卷，内卷标志：{auction_offer_flag}，当前等待时间：{auction_offer_all_count * AUCTIONOFFERSLEEPTIME}，总计内卷次数：{auction_offer_time_count}")
    if error_msg == '':
