...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    '灵根类型': f'{mess.root_type}(属性：{mess.root} 修炼速度提升{int(level_rate * 100)}%)',
    '神识强度': f'{int(sql_message.get_spirit_power(mess.level)*float(mess.spirit_rate))}(神魂天资：{mess.spirit_type} 神识强度提升{int(float(mess.spirit_rate) * 100)}%)',
...
灵根类型：{mess.root_type}(属性：{mess.root} 修炼速度提升{int(level_rate * 100)}%)
神识强度：{int(sql_message.get_spirit_power(mess.level)*float(mess.spirit_rate))}(神魂天资：{mess.spirit_type} 神识强度提升{int(float(mess.spirit_rate) * 100)}%)
